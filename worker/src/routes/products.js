...
import { requireAuth, requireAdmin, requireEditor, requireComerciante, authMiddleware } from '../middleware/auth.js';
...
export async function handleComercianteGetProducts(request, env) {
    const { error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
...
export async function handleComercianteGetStats(request, env) {
    const { error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
...
export async function handleComercianteCreateProduct(request, env) {
    const { user, error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
...
export async function handleComercianteUpdateProduct(request, env, id) {
    const { error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
...
export async function handleComercianteUpdateProductStatus(request, env, id) {
    const { error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
...
export async function handleComercianteDeleteProduct(request, env, id) {
    const { error: authError } = await requireComerciante(request, env);
    if (authError) return authError;
